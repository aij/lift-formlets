...
object FormValidation extends FormValidationInstances {
  ...
  /** Lifts a function of two parameters into the equivalent [[FormValidation]] */
  def lift2VS[A, B](f: (FormValue[B], A) => Validation[String, A]): FormValidation[(FormValue[B], A), A] =
    FormValidation({ case (bn, a) => liftStringV(f(bn, a)) }, None)
  ...
  /** Lifts a function of three parameters into the equivalent [[FormValidation]] */
  def lift3VS[A, B, C](
    f: (FormValue[B], FormValue[C], A) => Validation[String, A]
  ): FormValidation[(FormValue[B], FormValue[C], A), A] =
    FormValidation({ case (bn, cn, a) => liftStringV(f(bn, cn, a)) }, None)
  ...
  /** Lifts a function of four parameters into the equivalent [[FormValidation]] */
  def lift4VS[A, B, C, D](
    f: (FormValue[B], FormValue[C], FormValue[D], A) => Validation[String, A]
  ): FormValidation[(FormValue[B], FormValue[C], FormValue[D], A), A] =
    FormValidation({ case (bn, cn, dn, a) => liftStringV(f(bn, cn, dn, a)) }, None)
  ...
  object FormMetadata {
    def empty: FormMetadata = FormMetadata(None, None, None, Vector())
    ...
    private val metadataErrorContext = Lens.lensu[FormMetadata, Option[ErrorContext]](
      (a, value) => a.copy(errorContext = value),
      _.errorContext
    )

    private val metadataDescription = Lens.lensu[FormMetadata, Vector[String]](
      (a, value) => a.copy(description = value),
      _.description
    )
    ...
    def formErrorContext[A]: BoundForm[A] @> Option[ErrorContext] =
      BoundForm.formMetadata[A] >=> metadataErrorContext

    def formDescription[A]: BoundForm[A] @> Vector[String] =
      BoundForm.formMetadata[A] >=> metadataDescription
  }
  ...
  def mapStringV[B](f: A => Validation[String, B]): Form[B] =
    this.mapV(FormValidation(a => FormValidation.liftStringV(f(a)), None))
  ...
  /** Modifies the result of this form to have no error context */
  def clearErrorContext: Form[A] =
    mapResult(FormMetadata.formErrorContext.set(_, None))

  def describe(f: A => Vector[String]): Form[A] =
    mapResult(a => a.result.fold(_ => a, s => FormMetadata.formDescription.set(a, f(s))))
  ...
  def liftStringV[C](implicit ev: A <:< ValidationNelS[C]): Form[C] =
    mapResult(x => x.copy(result = x.result.fold(_.failure[C], a => FormValidation.liftNelStringV(a))))
}
...
trait FormInstances {
  implicit val formApplicative: Applicative[Form] = new Applicative[Form] {
    ...
    override def point[A](a: => A): Form[A] =
      Form.liftResult(BoundForm(a.success, FormMetadata.empty, cssSelZero))
...
object Form extends FormInstances {
  ...
  def sel(sel: CssSel, label: Option[String] = None): Form[Unit] =
    fresult { (_, _) => BoundForm(().success, FormMetadata(label, None, None, Vector()), sel) }
